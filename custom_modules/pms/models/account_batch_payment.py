...
class AccountBatchPayment(models.Model):
    ...
    def _generate_nacha_header(self):
        ...
        header.append("{:>10.10}".format(self.journal_id.nacha_immediate_destination))
        header.append("")  # Immediate Destination
    ...
    def _generate_nacha_batch_header_record(self, payment, batch_nr):
        ...
        batch.append("225")  # Service Class Code (credits only)
    ...
    def _generate_nacha_entry_detail(self, payment, batch_nr):
        def calculate_check_digit(routing_number):
            weights = [3, 7, 1]
            sum_ = sum(int(digit) * weights[i % 3] for i, digit in enumerate(routing_number[:8]))
            check_digit = (10 - (sum_ % 10)) % 10
            return str(check_digit)
        
        routing_number = self.decrypt_data(payment.routing_number)
        check_digit = calculate_check_digit(routing_number)

        bank = payment.partner_bank_id
        entry = []
        entry.append("6")  # Record Type Code (PPD)
        entry.append("27")  # Transaction Code
        entry.append("{:8.8}".format(self.decrypt_data(payment.routing_number)[:-1]))  # RDFI Routing Transit Number
        entry.append("{:1.1}".format(check_digit))  # Check Digit
        entry.append("{:17.17}".format(self.decrypt_data(payment.account_number)))  # DFI Account Number
        entry.append("{:010d}".format(round(payment.amount * 100)))  # Amount
        entry.append("{:15.15}".format(""))  # Individual Identification Number (optional)
        entry.append("{:22.22}".format(self.decrypt_data(payment.account_holder)))  # Individual Name
    ...
    def _generate_nacha_batch_control_record(self, payment, batch_nr):
        ...
        control.append("225")  # Service Class Code (credits only)
        control.append("{:06d}".format(1))  # Entry/Addenda Count
        control.append("{:010d}".format(self._calculate_aba_hash(self.decrypt_data(payment.routing_number))))  # Entry Hash
    ...
    def _generate_nacha_file_control_record(self, payments):
        ...
        hashes = sum(self._calculate_aba_hash(self.decrypt_data(payment.routing_number)) for payment in payments)
    ...
    def _generate_nacha_file(self):
        ...
        entries = []
        for batch_nr, payment in enumerate(self.payment_ids, start=1):
    ...
    def _generate_export_file(self):
        if self.payment_method_code == "new_ach_fast_payment":
