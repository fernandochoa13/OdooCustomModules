...
class CashPnlReportWizard(models.TransientModel):
    _name = 'cash.pnl.report.wizard'
    _description = 'Cash Profit & Loss Report Wizard'
    ...
    def action_open_report(self):
        ...
        return {
            ...
            'res_model': 'account.analytic.cash.pnl.report',
            'view_mode': 'pivot,tree',
            'name': 'Analytic P&L Report',
            'context': {
                'cash_pnl_year': self.year,
            },
        }


class AccountAnalyticCashPnlReport(models.Model):
    _name = 'account.analytic.cash.pnl.report'
    ...
    def _table_query(self):
        ...
        """
        tools.drop_view_if_exists(self.env.cr, 'account_analytic_cash_pnl_report')
        
        active_company_id = self.env.company.id
        # Get the year from the context, or default to the current year
        selected_year = self.env.context.get('cash_pnl_year', date.today().year)
        ...
        return f"""
            ...
            WHERE
                ...
                AND am.cash_date BETWEEN '{selected_year}-01-01' AND '{selected_year}-12-31'
